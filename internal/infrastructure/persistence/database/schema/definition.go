...
func SchemaMigrations(engine persistenceengine.Engine, schema string) ([]modulehost.Migration, error) {
	...
	workerScopesTable := profile.Table(schema, "_worker_scopes")
	...
	workerScopes := `CREATE TABLE IF NOT EXISTS ` + workerScopesTable + ` (` +
		`id ` + key + ` PRIMARY KEY, owner ` + key + ` NOT NULL, scope_key ` + key + ` NOT NULL, cursor ` + key + ` NOT NULL DEFAULT '', ` +
		`checkpoint BIGINT NOT NULL DEFAULT 0, capacity BIGINT NOT NULL DEFAULT 0, lease_owner ` + key + ` NOT NULL DEFAULT '', lease_expires_at ` + key + ` NOT NULL DEFAULT '', ` +
		`fencing_token BIGINT NOT NULL DEFAULT 0, last_started_at ` + key + ` NOT NULL DEFAULT '', last_completed_at ` + key + ` NOT NULL DEFAULT '', ` +
		`last_error ` + large + ` NOT NULL DEFAULT '', updated_at ` + key + ` NOT NULL DEFAULT '', UNIQUE(owner, scope_key))`
	...
	return []modulehost.Migration{
		...
		{ID: "data_exchange_worker_scopes_v1", SQL: workerScopes}, {ID: "data_exchange_job_owner_reference_v2", SQL: ownerReference},
